...
The DBSCAN algorithm has two parameters: the minimum points $\kappa$ (in some papers referred only with "minimum points"), which are required to initialize a new cluster and the radius $\epsilon$ in which neighbour points should be searched.
\\
...
\begin{figure}
	...
	{\caption{Plot of DBSCAN-filtered data}\label{fig:dbscan_plot1}}
\end{figure}
...
7: ball\\
\\
So if we want to know the state of a certain position we call that dictionary and get back a number between 0 and 7.
\\
